...
import { ArrowLeft, Edit, Eye, TrendingUp, Package, Calendar, Trash } from "lucide-react";
...
const AdminProductDetailPage = () => {
  ...
  const { deleteProduct, error: adminError, clearError } = useAdminProductStore();
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm(`Are you sure you want to delete ${productDetail.name}?`)) {
    ...
    <div className="space-y-6 container-custom py-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            onClick={() => navigate(`/admin/products/${productDetail.id}/edit`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-none shadow-sm">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {productDetail.product_image && productDetail.product_image.length > 0 ? (
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-lg font-medium">{productDetail.name}</p>
                </div>
                <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-medium">Rp {productDetail.price.toLocaleString("id-ID")}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Harga Diskon</p>
                    <p className="text-lg font-medium">
                      {productDetail.discount_price
                        ? `Rp ${productDetail.discount_price.toLocaleString("id-ID")}`
                        : "-"}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-lg font-medium">{productDetail.stock}</p>
                      <Badge className={getStatusBadge(productDetail.stock)}>
                        {productDetail.stock <= 10 ? "Low Stock" : "In Stock"}
                      </Badge>
                    </div>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Kategori</p>
                    <p className="text-lg font-medium">{productDetail.category?.name || "N/A"}</p>
                  </div>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Produk Hot</p>
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-lg font-medium">{productDetail.is_hot ? "Yes" : "No"}</p>
          ...
          <Card className="border-none shadow-sm">
            <CardHeader>
              <CardTitle>Reviews ({productDetail.reviews?.length || 0})</CardTitle>
            </CardHeader>
            <CardContent>
              {productDetail.reviews && productDetail.reviews.length > 0 ? (
                <div className="space-y-4">
                  {productDetail.reviews.slice(0, 5).map((review) => (
                  ...
                  {productDetail.reviews.length > 5 && (
                    <p className="text-sm text-gray-500 text-center">
                      And {productDetail.reviews.length - 5} more reviews...
                    </p>
                  )}
                </div>
              ) : (
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-lg font-medium">{productDetail.reviews?.length || 0}</p>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-lg font-medium">
                    {productDetail.reviews && productDetail.reviews.length > 0
                      ? (
                          productDetail.reviews.reduce((sum, review) => sum + review.rating, 0) /
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-gray-500">Stok Tersisa</p>
                  </div>
                  <p className="text-lg font-medium">{productDetail.stock}</p>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-sm font-medium">
                    {new Date(productDetail.updated_at).toLocaleDateString("id-ID")}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-2">
                <div>
                  ...
                  <p className="text-sm font-mono text-gray-700 break-all">{productDetail.id}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Category ID</p>
                  <p className="text-sm font-mono text-gray-700 break-all">{productDetail.category_id}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Created At</p>
                  <p className="text-sm text-gray-700">
                    {new Date(productDetail.created_at).toLocaleDateString("id-ID", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                      hour: "2-digit",
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700">{productDetail.product_image?.length || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-2">
                <Button
                  variant="outline"
                  className="w-full"
                  onClick={() => window.open(`/products/${productDetail.id}`, "_blank")}
                >
