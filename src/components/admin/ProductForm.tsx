...
export const ProductForm: React.FC<ProductFormProps> = ({ product, onClose }) => {
  const { categories, createProduct, updateProduct, getCategories } = useProductStore();
  ...
  const [imagePreviews, setImagePreviews] = useState<string[]>(product?.product_image?.map(img => img.image_url) || []);
    ...
    <div className="max-h-[80vh] overflow-y-auto">
      <form onSubmit={handleSubmit} className="space-y-6 p-1">
        ...
        <div>
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            {imagePreviews.map((preview, index) => (
              <img key={index} src={preview} alt="Preview" className="w-full h-24 object-cover rounded" />
            ))}
          </div>
        </div>
        <div className="flex space-x-2 sticky bottom-0 bg-white pt-4 border-t">
          <Button
            type="submit"
            disabled={isLoadingKey('create-product') || isLoadingKey('update-product')}
          >
            {isLoadingKey('create-product') || isLoadingKey('update-product')
              ? 'Saving...'
              : product
              ? 'Update Product'
              : 'Create Product'}
