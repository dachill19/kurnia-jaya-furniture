...
const EditReviewPage = () => {
  ...
  const { getReviewByOrderItemId, updateReview, error: reviewError, clearError } = useReviewStore();
      ...
      if (reviewId) {
        try {
          ...
          if (review.data.review_image) {
            setExistingImages(review.data.review_image.map((img) => img.image_url));
            setPreviewImages(review.data.review_image.map((img) => img.image_url));
          }
        } catch (error) {
          toast({
            title: "Error",
  ...
  const handleRemoveImage = (index: number) => {
    ...
    setExistingImages((prev) => prev.filter((_, i) => i !== index % existingImages.length));
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Ulasan tidak ditemukan.</p>
  ...
  return (
    <div className="space-y-6 container-custom py-8">
      ...
      <Card className="border-none shadow-sm">
        <CardHeader>
          <CardTitle>Formulir Edit Ulasan</CardTitle>
        </CardHeader>
        <CardContent>
